...
class Agent():
    ...
    def act(self, state, remaining):
        with torch.no_grad():
            state = torch.tensor(state[:, -1, :], dtype=torch.float32, device='cuda')
            state = state.flatten()
            remaining = torch.tensor(remaining, dtype=torch.float32, device='cuda')
            return (self.online_net(state.unsqueeze(0), remaining.view(1, 1)) * self.support).sum(2).argmax(1).item()

    # Acts with an ε-greedy policy (used for evaluation only)
    def act_e_greedy(self, state, epsilon=0.001):  # High ε can reduce evaluation scores drastically
        return np.random.randint(0, self.action_space) if np.random.random() < epsilon else self.act(state)

    def learn(self, mem, size):
        # Sample transitions
        idxs, states, actions, returns, next_states, nonterminals, weights, timesteps = mem.sample(self.batch_size)

        # Calculate current state probabilities (online network noise already sampled)
        log_ps = self.online_net(states, (size - timesteps[:, 0]).view(-1, 1),
                                 log=True)  # Log probabilities log p(s_t, ·; θonline)
        log_ps_a = log_ps[range(self.batch_size), actions]  # log p(s_t, a_t; θonline)

        with torch.no_grad():
            # Calculate nth next state probabilities
            t = size - timesteps[:, -1].view(-1, 1)
            pns = self.online_net(next_states, t)  # Probabilities p(s_t+n, ·; θonline)
            ...
            pns = self.target_net(next_states, t)  # Probabilities p(s_t+n, ·; θtarget)
            pns_a = pns[range(
    ...
    def evaluate_q(self, state, t):
        with torch.no_grad():
            return (self.online_net(state.unsqueeze(0), t.view(1, 1)) * self.support).sum(2).max(1)[0].item()
